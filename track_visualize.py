...
def track_chunks(cotracker, vis, video, video_chunk_max_len, threshold_perc, grid_size):
    video = torch.from_numpy(np.stack(video)).cuda().float()
    ...
    for video_chunk in video_chunks:
        ...
        track_video = vis.visualize(
            video_chunk,
            pred['tracks'][0][-1].unsqueeze(0), 
            pred['visibility'][0][-1].unsqueeze(0),
            save_video=False,
        )
        track_videos.append(track_video)
    track_video = torch.cat(track_videos, dim=1)
    video_frames = []
    for frame_id in range(track_video.shape[1]):
        video_frames.append(track_video[0, frame_id].permute(1, 2, 0).to(dtype=torch.uint8).numpy()) 
    return pred, video_frames
